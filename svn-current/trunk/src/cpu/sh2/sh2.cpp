...
int Sh2MapMemory(unsigned char *pMemory, unsigned int nStart, unsigned int nEnd, int nType)
{
	...
	for (unsigned long long i = (nStart & ~SH2_PAGEM); i <= nEnd; i += SH2_PAGE_SIZE, pMemMap++)
	{
		...
		if (nType & 0x02 /*SM_WRITE*/)
			pMemMap[SH2_WADD] = Ptr + i;
		if (nType & 0x04 /*SM_FETCH*/)
			pMemMap[SH2_WADD * 2] = Ptr + i;
...
int Sh2MapHandler(uintptr_t nHandler, unsigned int nStart, unsigned int nEnd, int nType)
{
	...
	for (unsigned long long i = (nStart & ~SH2_PAGEM); i <= nEnd; i += SH2_PAGE_SIZE, pMemMap++)
	{
		...
		if (nType & 0x02 /*SM_WRITE*/)
			pMemMap[SH2_WADD] = (unsigned char *)nHandler;
		if (nType & 0x04 /*SM_FETCH*/)
			pMemMap[SH2_WADD * 2] = (unsigned char *)nHandler;
...
#if defined(VITA) && defined(DRC_SH2)
	// Ensure a valid SH2 pointer before initializing DRC
	pSh2Ext = Sh2Ext;
	sh2 = &pSh2Ext->sh2;
	Sh2DrcInit();
#endif
...
#if defined(VITA) && defined(DRC_SH2)
	// Reset DRC cache when SH2 is reset
	extern void Sh2DrcReset();
	Sh2DrcReset();
#endif
}
...
#define CHECK_PENDING_IRQ(/*message*/)                                          \
	...
	{                                                                           \
		int irq = -1;                                                           \
		if (sh2->pending_irq & (1 << 0))                                        \
			irq = 0;                                                            \
		if (sh2->pending_irq & (1 << 1))                                        \
			irq = 1;                                                            \
		if (sh2->pending_irq & (1 << 2))                                        \
			irq = 2;                                                            \
		if (sh2->pending_irq & (1 << 3))                                        \
			irq = 3;                                                            \
		if (sh2->pending_irq & (1 << 4))                                        \
			irq = 4;                                                            \
		if (sh2->pending_irq & (1 << 5))                                        \
			irq = 5;                                                            \
		if (sh2->pending_irq & (1 << 6))                                        \
			irq = 6;                                                            \
		if (sh2->pending_irq & (1 << 7))                                        \
			irq = 7;                                                            \
		if (sh2->pending_irq & (1 << 8))                                        \
			irq = 8;                                                            \
		if (sh2->pending_irq & (1 << 9))                                        \
			irq = 9;                                                            \
		if (sh2->pending_irq & (1 << 10))                                       \
			irq = 10;                                                           \
		if (sh2->pending_irq & (1 << 11))                                       \
			irq = 11;                                                           \
		if (sh2->pending_irq & (1 << 12))                                       \
			irq = 12;                                                           \
		if (sh2->pending_irq & (1 << 13))                                       \
			irq = 13;                                                           \
		if (sh2->pending_irq & (1 << 14))                                       \
			irq = 14;                                                           \
		if (sh2->pending_irq & (1 << 15))                                       \
			irq = 15;                                                           \
		if ((sh2->internal_irq_level != -1) && (sh2->internal_irq_level > irq)) \
			irq = sh2->internal_irq_level;                                      \
		if (irq >= 0)                                                           \
...
SH2_INLINE void op1000(UINT16 opcode)
{
	...
	{
	case 0 << 8:
		MOVBS4(opcode & 0x0f, Rm);
		break;
	case 1 << 8:
		MOVWS4(opcode & 0x0f, Rm);
		break;
	case 2 << 8:
	case 3 << 8:
		NOP();
		break;
	case 4 << 8:
		MOVBL4(Rm, opcode & 0x0f);
		break;
	case 5 << 8:
		MOVWL4(Rm, opcode & 0x0f);
		break;
	case 6 << 8:
	case 7 << 8:
		NOP();
...
#ifdef DRC_SH2
static void sh2_update_drc_pending_level(void)
{
    int level = -1;
    for (int i = 15; i >= 0; i--) {
