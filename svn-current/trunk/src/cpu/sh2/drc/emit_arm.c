...
// XXX: tcache_ptr type for SVP and SH2_DRC compilers differs..
...
static NOINLINE void EMIT(u32 op, u32 dst, u32 src)
{
	...
	emit_cache[n] = (struct emit_op){.op = op, .src = src & ~M1(NO), .dst = dst & ~M1(NO)};
...
#define emith_move_r_imm_s8_patch(ptr, imm)                       \
	do                                                            \
	{                                                             \
		u32 *ptr_ = (u32 *)ptr;                                   \
		u32 op_ = *ptr_ & 0xfe1ff000;                             \
		if ((s8)(imm) < 0)                                        \
...
#define emith_jump_patch(ptr, target, pos)                  \
	do                                                      \
	{                                                       \
		u32 *ptr_ = (u32 *)ptr;                             \
...
#define emith_sh2_rcall(a, tab, func, mask)                       \
	...
	{                                                             \
		...
		if (func < mask)                                          \
			EOP_LDMIA(tab, M2(func, mask)); /* ldm if possible */ \
		else                                                      \
		{                                                         \
			emith_read_r_r_offs(func, tab, 0);                    \
...
#define emith_sh2_wcall(a, val, tab, func)                               \
	do                                                                   \
	{                                                                    \
		printf("SH2DRC: emith_sh2_wcall tab=r%d func=r%d\n", tab, func); \
...
#define emith_sh2_delay_loop(cycles, reg)                     \
	do                                                        \
	{                                                         \
...
#define emith_t_to_carry(srr, is_sub)      \
	do                                     \
	{                                      \
		if (is_sub)                        \
		{                                  \
...
#define emith_sh2_macl(ml, mh, rn, rm, sr)                                     \
	do                                                                         \
	{                                                                          \
...
/* mh:ml += rn*rm, does saturation if required by S bit. rn, rm must be TEMP */
#define emith_sh2_macw(ml, mh, rn, rm, sr)                                      \
	do                                                                          \
	{                                                                           \
		emith_tst_r_imm(sr, S);                                                 \
		EMITH_SJMP2_START(DCOND_NE);                                            \
		...
		/* XXX: LSB signalling only in SH1, or in SH2_DRC too? */               \
