...
#ifndef DRC_DEBUG
// DRC_DEBUG already defined above, don't redefine
...
#define FETCH32(a) \
  (p32x_sh2_read32((a), drc_fetch_sh2))
...
union _div
{
  u32 imm;
  struct div div;
}; // XXX tut-tut type punning...
#define div(opd) ((union _div *)&((opd)->imm))->div

// XXX consider trap insns: OP_TRAPA, OP_UNDEFINED?
#define OP_ISBRANCH(op) ((BITRANGE(OP_BRANCH, OP_BRANCH_RF) | BITMASK1(OP_RTE)) & BITMASK1(op))
#define OP_ISBRAUC(op) (BITMASK4(OP_BRANCH, OP_BRANCH_R, OP_BRANCH_RF, OP_RTE) & BITMASK1(op))
#define OP_ISBRACND(op) (BITMASK2(OP_BRANCH_CT, OP_BRANCH_CF) & BITMASK1(op))
#define OP_ISBRAIMM(op) (BITMASK3(OP_BRANCH, OP_BRANCH_CT, OP_BRANCH_CF) & BITMASK1(op))
#define OP_ISBRAIND(op) (BITMASK3(OP_BRANCH_R, OP_BRANCH_RF, OP_RTE) & BITMASK1(op))

#ifdef DRC_SH2

// Disassembly buffer - always needed
...
static void REGPARM(3) * sh2_drc_log_entry(void *block, SH2_DRC *sh2, u32 sr)
{
  if (block != NULL)
  {
#if defined PDB
    ...
    if (lastpc != sh2->pc)
    {
      ...
      lastcnt++;
#endif
    // Always dump if PC looks invalid
    if (sh2->pc >= 0xF0000000 || sh2->pc == 0)
    {
      printf("SH2DRC: INVALID PC %08x at block entry!\n", sh2->pc);
...
#elif (DRC_DEBUG & 256)
    {
      ...
      {
        fwrite(sh2, offsetof(SH2_DRC, read8_map), 1, trace[idx]);
        fwrite(&sh2->pdb_io_csum, sizeof(sh2->pdb_io_csum), 1, trace[idx]);
        memcpy(&csh2[idx][0], sh2, offsetof(SH2_DRC, poll_cnt) + 4);
...
#elif (DRC_DEBUG & 512)
    {
      ...
      if (trace[idx] && csh2[idx][0].pc != sh2->pc)
      {
        if (!fread(&fsh2, offsetof(SH2_DRC, read8_map), 1, trace[idx]) ||
        ...
        fsh2.is_slave = idx;
        if (memcmp(&fsh2, sh2, offsetof(SH2_DRC, read8_map)) ||
            0) // memcmp(&fsh2.pdb_io_csum, &sh2->pdb_io_csum, sizeof(sh2->pdb_io_csum)))
        {
          ...
          for (idx = 0; idx < offsetof(SH2_DRC, read8_map); idx += sizeof(u32))
            if (*(u32 *)(ps + idx) != *(u32 *)(pf + idx))
              printf("diff reg %ld\n", (long)idx / sizeof(u32));
          exit(1);
        }
        memcpy(&csh2[idx][1], &csh2[idx][0], offsetof(SH2_DRC, poll_cnt) + 4);
        csh2[idx][0] = fsh2;
      }
    }
#elif (DRC_DEBUG & 1024)
    {
      ...
      for (i = 0; i < ARRAY_SIZE(csh2[x]) - 1; i++)
        memcpy(&csh2[x][i], &csh2[x][i + 1], offsetof(SH2_DRC, poll_cnt) + 4);
      memcpy(&csh2[x][ARRAY_SIZE(csh2[x]) - 1], sh2, offsetof(SH2_DRC, poll_cnt) + 4);
      csh2[x][0].is_slave = x;
    }
#endif
  }
...
struct block_desc
{
  u32 addr;       // block start SH2_DRC PC address
  u32 addr_lit;   // block start SH2_DRC literal pool addr
...
u8 *tcache_ptr; // ptr for code emitters (extern in cmn.h)
...
static void REGPARM(1) (*sh2_drc_entry)(SH2_DRC *sh2);
#ifdef DRC_SR_REG
void REGPARM(1) (*sh2_drc_save_sr)(SH2_DRC *sh2);
void REGPARM(1) (*sh2_drc_restore_sr)(SH2_DRC *sh2);
#endif
...
// address space stuff
static int dr_ctx_get_mem_ptr(SH2_DRC *sh2, u32 a, u32 *mask)
{
  void *memptr;
  int poffs = -1;
  ...
  if (memptr == sh2->p_bios) // BIOS
    ...
    poffs = offsetof(SH2_DRC, p_da);
  else if (memptr == sh2->p_sdram) // SDRAM
    poffs = offsetof(SH2_DRC, p_sdram);
  else if (memptr == sh2->p_rom) // ROM
    poffs = offsetof(SH2_DRC, p_rom);
...
static void dr_block_link(struct block_entry *be, struct block_link *bl, int emit_jump)
{
  ...
  if (emit_jump)
  {
    ...
    else if (bl->type == BL_LDJMP)
    { // write: jump @entry
      // inlined: @jump far jump to target
      emith_jump_at(jump, be->tcache_ptr);
      jsz = emith_jump_at_size();
    }
    else if (bl->type == BL_JCCBLX)
    { // patch: jump cond -> jump @entry
...
static void dr_mark_memory(int mark, struct block_desc *block, int tcache_id, u32 nolit)
{
  ...
  if ((block->addr & 0xc7fc0000) == 0x06000000 || (block->addr & 0xfffff000) == 0xc0000000)
  {
    if (tcache_id != 0)
    {
      // data array
...
static u32 dr_check_nolit(u32 start, u32 end, int tcache_id)
{
  ...
  if ((start & 0xc7fc0000) == 0x06000000 || (start & 0xfffff000) == 0xc0000000)
  {
    if (tcache_id != 0)
    {
      // data array
    ...
    return (addr < start ? start : addr > end ? end
                                              : addr);
  }
...
static void rcache_set_x16(int hr, int s16_, int u16_)
{
  ...
  if (x >= 0)
  {
    ...
    if (s16_)
      cache_regs[x].flags |= HRF_S16;
    if (u16_)
      cache_regs[x].flags |= HRF_U16;
  }
}
...
#define RCACHE_CHECK(msg)                                                                                              \
  {                                                                                                                    \
    ...
    {                                                                                                                  \
      cp = &cache_regs[i];                                                                                             \
      if (cp->flags & HRF_PINNED)                                                                                      \
        m |= (1 << i);                                                                                                 \
      if (cp->type == HR_FREE || cp->type == HR_TEMP)                                                                  \
        continue;                                                                                                      \
      /* check connectivity greg->vreg */                                                                              \
      FOR_ALL_BITS_SET_DO(cp->gregs, x, if (guest_regs[x].vreg != i) { d = 1; printf("cache check v=%d r=%d not connected?\n",i,x); })                                              \
    }                                                                                                                  \
    for (i = 0; i < ARRAY_SIZE(guest_regs); i++)                                                                       \
    {                                                                                                                  \
      gp = &guest_regs[i];                                                                                             \
      ...
      {                                                                                                                \
        d = 1;                                                                                                         \
        printf("cache check r=%d v=%d not connected?\n", i, gp->vreg);                                                 \
      }                                                                                                                \
      if (gp->vreg != -1 && cache_regs[gp->vreg].type != HR_CACHED)                                                    \
      {                                                                                                                \
        d = 1;                                                                                                         \
        printf("cache check r=%d v=%d wrong type?\n", i, gp->vreg);                                                    \
      }                                                                                                                \
      if ((gp->flags & GRF_CONST) && !(gconsts[gp->cnst].gregs & (1 << i)))                                            \
      {                                                                                                                \
        d = 1;                                                                                                         \
        printf("cache check r=%d c=%d not connected?\n", i, gp->cnst);                                                 \
      }                                                                                                                \
      if ((gp->flags & GRF_CDIRTY) && (gp->vreg != -1 || !(gp->flags & GRF_CONST)))                                    \
      {                                                                                                                \
        d = 1;                                                                                                         \
        printf("cache check r=%d CDIRTY?\n", i);                                                                       \
      }                                                                                                                \
      ...
      {                                                                                                                \
        ...
        {                                                                                                              \
          d = 1;                                                                                                       \
          printf("cache check r=%d v=%d not pinned?\n", i, gp->vreg);                                                  \
        }                                                                                                              \
        else                                                                                                           \
          m &= ~(1 << gp->sreg);                                                                                       \
      }                                                                                                                \
    ...
    for (i = 0; i < ARRAY_SIZE(gconsts); i++)                                                                          \
    {                                                                                                                  \
      FOR_ALL_BITS_SET_DO(gconsts[i].gregs, x, if (guest_regs[x].cnst != i || !(guest_regs[x].flags & GRF_CONST)) { d = 1; printf("cache check c=%d v=%d not connected?\n",i,x); }) \
    }                                                                                                                  \
    if (m)                                                                                                             \
    {                                                                                                                  \
      d = 1;                                                                                                           \
      printf("cache check m=%x pinning wrong?\n", m);                                                                  \
    }                                                                                                                  \
    if (d)                                                                                                             \
      RCACHE_DUMP(msg)                                                                                                 \
    /*  else {                                                                                                         \
...
// SH2_DRC register usage bitmasks
...
static void rcache_unmap_vreg(int x)
{
  ...
  FOR_ALL_BITS_SET_DO(cache_regs[x].gregs, i, if (guest_regs[i].flags & GRF_DIRTY) {
        ...
        guest_regs[i].flags &= ~GRF_DIRTY; } guest_regs[i].vreg = -1);
  rcache_free_vreg(x);
}
...
static void rcache_clean_vreg(int x)
{
  ...
  if (cache_regs[x].flags & HRF_DIRTY)
  { // writeback
    cache_regs[x].flags &= ~HRF_DIRTY;
    rcache_lock_vreg(x);
    FOR_ALL_BITS_SET_DO(cache_regs[x].gregs, r, if (guest_regs[r].flags & GRF_DIRTY) {
  ...
  for (i = ARRAY_SIZE(cache_regs) - 1; i >= 0; i--)
  {
    ...
    if (cache_regs[i].type == HR_CACHED)
    {
      if (rcache_regs_now & cache_regs[i].gregs)
        ...
        i_prio = 8;
      if (!(cache_regs[i].flags & HRF_DIRTY))
        i_prio++;
...
static void rcache_remap_vreg(int x)
{
  ...
  FOR_ALL_BITS_SET_DO(cache_regs[x].gregs, d, if ((guest_regs[d].flags & (GRF_STATIC | GRF_PINNED)) && !cache_regs[guest_regs[d].sreg].locked && !((rsl_d | rcache_regs_now) & cache_regs[guest_regs[d].sreg].gregs)) {
  ...
  if (mode != RC_GR_READ && src == dst && ali)
  {
    ...
    if ((rsp_d | rcache_regs_now) & ali)
    {
          ...
          guest_regs[r].sreg == dst && !tr->locked)
      {
        ...
        int t;
        FOR_ALL_BITS_SET_DO(ali, t, if ((guest_regs[t].flags & (GRF_STATIC | GRF_PINNED)) && !(ali & ~(1 << t)) && !cache_regs[guest_regs[t].sreg].locked && !((rsp_d | rcache_regs_now) & cache_regs[guest_regs[t].sreg].gregs)) {
            // alias is a single STATIC and its sreg is available
            x = guest_regs[t].sreg;
            rcache_evict_vreg(x); } else {
            rcache_lock_vreg(dst); // lock to avoid evicting dst
            x = rcache_allocate_vreg(rsp_d & ali);
            rcache_unlock_vreg(dst); } break;)
        if (x >= 0)
        {
          rcache_remove_vreg_alias(src, r);
          src = dst;
...
static int rcache_save_tmp(int hr)
{
  ...
  if (i < 0)
  {
    ...
    emith_ctx_write(hr, offsetof(SH2_DRC, drc_tmp));
    rcache_free_tmp(hr);
    return -1;
  }
...
static int rcache_restore_tmp(int x)
{
  ...
  hr = rcache_get_tmp();
  emith_ctx_read(hr, offsetof(SH2_DRC, drc_tmp));
...
static void rcache_clean_masked(u32 mask)
{
  ...
  for (i = 0; i < ARRAY_SIZE(gconsts); i++)
    ...
    {
      FOR_ALL_BITS_SET_DO(gconsts[i].gregs, r, if (guest_regs[r].flags & GRF_CDIRTY) {
...
static void rcache_create(void)
{
  ...
  // create static host register mapping for SH2_DRC regs
  for (i = 0; i < ARRAY_SIZE(guest_regs); i++)
  {
...
static uptr split_address(uptr la, uptr mask, s32 *offs)
{
  ...
  *offs = (la & mask) | (la & sign ? ~mask : 0); // offset part, sign extended
  ...
  if (*offs < 0)
  { // TODO not working at all with negative offsets on ARM?
    // if (*offs == -sign) {
    la -= sign;
    *offs += sign;
  }
...
static int emit_get_rbase_and_offs(SH2_DRC *sh2, sh2_reg_e r, int rmode, s32 *offs)
{
  ...
  if (mask < 0x20000)
  {
    ...
    hr = rcache_get_tmp();
    a = (a + *offs) & mask;
    if (poffs == offsetof(SH2_DRC, p_da))
    {
      // access sh2->data_array directly
      a = split_address(a + offsetof(SH2_DRC, data_array), omask, offs);
  ...
  // ROM, SDRAM. Host address should be mmapped to be equal to SH2_DRC address.
...
static int emit_get_rom_data(SH2_DRC *sh2, sh2_reg_e r, s32 offs, int size, u32 *val)
{
  u32 a, mask;
...
static int emit_memhandler_read(int size)
{
  ...
  if (size & MF_POLLING)
    ...
    {
    case 0:
      emith_call(sh2_drc_read8_poll);
      break; // 8
    case 1:
      emith_call(sh2_drc_read16_poll);
      break; // 16
    case 2:
      emith_call(sh2_drc_read32_poll);
      break; // 32
    }
  else
    switch (size & MF_SIZEMASK)
...
static int emit_memhandler_read_rr(SH2_DRC *sh2, sh2_reg_e rd, sh2_reg_e rs, s32 offs, int size)
{
  int hr, hr2;
  u32 val;
  ...
  if (hr != -1)
  {
    ...
    {
    case 0:
      emith_read8s_r_r_offs(hr2, hr, MEM_BE2(offs));
      break; // 8
    case 1:
      emith_read16s_r_r_offs(hr2, hr, offs);
      break; // 16
    case 2:
      emith_read_r_r_offs(hr2, hr, offs);
      emit_le_swap(-1, hr2);
      break;
    }
    rcache_free(hr);
...
static void emit_memhandler_write_rr(SH2_DRC *sh2, sh2_reg_e rd, sh2_reg_e rs, s32 offs, int size)
{
  int hr, hr2;
  u32 val;
  ...
  else if ((size & MF_PREDECR) && rd == rs)
  { // must avoid caching rd in arg1
...
static int emit_indirect_indexed_read(SH2_DRC *sh2, sh2_reg_e rd, sh2_reg_e rx, sh2_reg_e ry, int size)
{
  int hr, hr2;
  int tx, ty;
#if PROPAGATE_CONSTANTS
...
static void emit_indirect_indexed_write(SH2_DRC *sh2, sh2_reg_e rd, sh2_reg_e rx, sh2_reg_e ry, int size)
{
  int hr, tx, ty;
#if PROPAGATE_CONSTANTS
  u32 offs;
...
// @Rn+,@Rm+
static void emit_indirect_read_double(SH2_DRC *sh2, int *rnr, int *rmr, sh2_reg_e rn, sh2_reg_e rm, int size)
{
  int tmp;
...
// 32:16 cases and the 64:32 cases described in the SH2_DRC prog man are replaced.

// This is surprisingly difficult since the SH2_DRC division operation is generating
...
static void emit_branch_linkage_code(SH2_DRC *sh2, struct block_desc *block, int tcache_id,
...
static void REGPARM(2) * sh2_translate(SH2_DRC *sh2, int tcache_id)
{
  ...
  if (sh2->pc >= 0xF0000000 || sh2->pc == 0)
  {
    printf("SH2DRC: ERROR - Attempting to translate invalid PC %08x\n", sh2->pc);
    printf("  R0-R3: %08x %08x %08x %08x\n", sh2->r[0], sh2->r[1], sh2->r[2], sh2->r[3]);
    printf("  R4-R7: %08x %08x %08x %08x\n", sh2->r[4], sh2->r[5], sh2->r[6], sh2->r[7]);
    printf("  R8-R11:%08x %08x %08x %08x\n", sh2->r[8], sh2->r[9], sh2->r[10], sh2->r[11]);
    printf("  R12-R14:%08x %08x %08x\n", sh2->r[12], sh2->r[13], sh2->r[14]);
    printf("  R15(SP):%08x\n", sh2->r[15]);
    printf("  PR=%08x SR=%08x GBR=%08x VBR=%08x\n", sh2->pr, sh2->sr, sh2->gbr, sh2->vbr);
    printf("  MACH=%08x MACL=%08x\n", sh2->mach, sh2->macl);
    printf("Stack Dump:\n");
    for (int i = 0; i < 16; i += 4)
    {
      printf("  %08x: %08x\n", sh2->r[15] + i, p32x_sh2_read32(sh2->r[15] + i, sh2));
    }
    return NULL;
  }
  ...
  // PC of current, first, last SH2_DRC insn
  ...
  // normally since the SH2_DRC code wasn't overwritten and should be the same.
  ...
  for (pc = base_pc, i = 0; pc < end_pc; i++, pc += 2)
  {
    ...
    if (ops[i].source & BITMASK1(SHR_T))
      ops[i].source |= BITMASK1(SHR_SR);
    if (ops[i].dest & BITMASK1(SHR_T))
      ops[i].source |= BITMASK1(SHR_SR);
    if (ops[i].dest & BITMASK1(SHR_T))
      ops[i].dest |= BITMASK1(SHR_SR);
#if LOOP_DETECTION
    // loop types detected:
    ...
    if (drcf.loop_type)
    {
      ...
      else if (ops[i].op != OP_LOAD_POOL && ops[i].op != OP_LOAD_CONST && (ops[i].op != OP_MOVE || op != OF_POLL_LOOP))
      {
  ...
  for (i = 0; pc < end_pc; i++)
  {
    ...
    for (v = 1; v <= 9; v++)
    {
      ...
      if (pc + 2 * v <= end_pc && !tmp)
      { // (pc already incremented above)
    ...
    case OP_LOAD_POOL:
#if PROPAGATE_CONSTANTS
      if (base_pc < 0x06000840 || base_pc > 0x06000880)
      {
      ...
      case 0x09:
        ...
        case 1: // DIV0U      0000000000011001
          ...
          if (div(opd).div1 == 16 && div(opd).ro == div(opd).rn)
          {
            ...
            tmp = rcache_get_tmp_arg(1);
            emith_add_r_r_ptr_imm(tmp, CONTEXT_REG, offsetof(SH2_DRC, drc_tmp));
            rcache_invalidate_tmp();
            ...
            emith_ctx_read(tmp3, offsetof(SH2_DRC, drc_tmp));
          ...
          else if (div(opd).div1 == 32 && div(opd).ro != div(opd).rn)
          {
            ...
            emith_ctx_write(tmp4, offsetof(SH2_DRC, drc_tmp));
            rcache_free(tmp4);
            rcache_get_reg_arg(0, div(opd).rn, NULL);
            rcache_get_reg_arg(2, div(opd).rm, NULL);
            tmp = rcache_get_tmp_arg(1);
            ...
            if (tmp != tmp2)
              emith_move_r_r(tmp2, tmp);
            emith_ctx_read(tmp4, offsetof(SH2_DRC, drc_tmp));
    ...
    case 0x02:
      ...
      case 0x07: // DIV0S Rm,Rn         0010nnnnmmmm0111
        ...
        if (div(opd).div1 == 16 && div(opd).ro == div(opd).rn)
        {
          ...
          emith_add_r_r_ptr_imm(tmp3, CONTEXT_REG, offsetof(SH2_DRC, drc_tmp));
          rcache_invalidate_tmp();
  ...
  if ((sh2->pc & 0xc6000000) == 0x02000000)
  { // ROM
...
static void sh2_generate_utils(void)
{
  ...
  emith_ctx_read_ptr(arg2, offsetof(SH2_DRC, write8_tab));
  emith_sh2_wcall(arg0, arg1, arg2, arg3);
  emith_flush();

  // sh2_drc_write16(u32 a, u32 d)
  sh2_drc_write16 = (void *)tcache_ptr;
  emith_ctx_read_ptr(arg2, offsetof(SH2_DRC, write16_tab));
  emith_sh2_wcall(arg0, arg1, arg2, arg3);
  emith_flush();

  // sh2_drc_write32(u32 a, u32 d)
  sh2_drc_write32 = (void *)tcache_ptr;
  emith_ctx_read_ptr(arg2, offsetof(SH2_DRC, write32_tab));
  emith_sh2_wcall(arg0, arg1, arg2, arg3);
  emith_flush();

  // d = sh2_drc_read8(u32 a)
  sh2_drc_read8 = (void *)tcache_ptr;
  emith_ctx_read_ptr(arg1, offsetof(SH2_DRC, read8_map));
  EMITH_HINT_COND(DCOND_CS);
  ...
  emith_ctx_read_ptr(arg1, offsetof(SH2_DRC, read16_map));
  EMITH_HINT_COND(DCOND_CS);
  emith_sh2_rcall(arg0, arg1, arg2, arg3);
  EMITH_SJMP_START(DCOND_CS);
  emith_and_r_r_c(DCOND_CC, arg0, arg3);
  ...
  emith_ctx_read_ptr(arg1, offsetof(SH2_DRC, read32_map));
  EMITH_HINT_COND(DCOND_CS);
  emith_sh2_rcall(arg0, arg1, arg2, arg3);
  EMITH_SJMP_START(DCOND_CS);
  emith_and_r_r_c(DCOND_CC, arg0, arg3);
...
#endif
  emith_read_r_r_offs_ptr_c(DCOND_EQ, RET_REG, arg1, offsetof(SH2_DRC, branch_cache) + sizeof(void *));
  emith_jump_reg_c(DCOND_EQ, RET_REG);
  EMITH_SJMP_END(DCOND_NE);
#endif
  emith_move_r_r_ptr(arg1, CONTEXT_REG);
  emith_add_r_r_ptr_imm(arg2, CONTEXT_REG, offsetof(SH2_DRC, drc_tmp));
  ...
  emith_ctx_read(arg1, offsetof(SH2_DRC, drc_tmp)); // tcache_id
  ...
  emith_write_r_r_offs_ptr(arg1, arg3, offsetof(SH2_DRC, rts_cache) + sizeof(void *));
  emith_write_r_r_offs(arg2, arg3, offsetof(SH2_DRC, rts_cache));
  rcache_flush();
  emith_ret();
  emith_flush();
  ...
  emith_read_r_r_offs(arg3, arg1, offsetof(SH2_DRC, rts_cache));
  ...
  emith_ctx_read(arg1, offsetof(SH2_DRC, pending_level));
  sr = rcache_get_reg(SHR_SR, RC_GR_READ, NULL);
  ...
  rcache_flush();
  emith_move_r_r_ptr(arg0, CONTEXT_REG);
  emith_abicall_ctx(offsetof(SH2_DRC, irq_callback)); // vector = sh2->irq_callback(sh2, level);
  // obtain new PC
  ...
  // sh2_drc_restore_sr(SH2_DRC *sh2)
...
#define MAKE_READ_WRAPPER(func)                               \
  {                                                           \
    ...
    emith_push_ret();                                         \
    emith_call(func);                                         \
    emith_ctx_read(arg2, offsetof(SH2_DRC, pdb_io_csum[0]));  \
    emith_addf_r_r(arg2, arg0);                               \
    emith_ctx_write(arg2, offsetof(SH2_DRC, pdb_io_csum[0])); \
    emith_ctx_read(arg2, offsetof(SH2_DRC, pdb_io_csum[1]));  \
    emith_adc_r_imm(arg2, 0x01000000);                        \
    emith_ctx_write(arg2, offsetof(SH2_DRC, pdb_io_csum[1])); \
    emith_pop_and_ret();                                      \
    emith_flush();                                            \
    func = tmp;                                               \
  }
...
void sh2_drc_wcheck_ram(u32 a, unsigned len, SH2_DRC *sh2)
{
  sh2_smc_rm_blocks(a, len, 0, 0);
}

void sh2_drc_wcheck_da(u32 a, unsigned len, SH2_DRC *sh2)
{
  sh2_smc_rm_blocks(a, len, 1 + sh2->is_slave, 0);
}

int sh2_execute_drc(SH2_DRC *sh2c, int cycles)
{
  int ret_cycles;

  // cycles are kept in SHR_SR unused bits (upper 20)
  // bit11 contains T saved for delay slot
  // others are usual SH2_DRC flags
  sh2c->sr &= 0x3f3;
  sh2c->sr |= (cycles - 1) << 12;
#if (DRC_DEBUG & 8)
  lastpc = lastcnt = 0;
  ...
  host_call(sh2_drc_entry, (SH2_DRC *))(sh2c);
  sh2c->state &= ~SH2_IN_DRC;
...
void sh2_drc_get_entry_points(uintptr_t *entry, uintptr_t *dispatcher, uintptr_t *exitp)
{
  if (entry)
    *entry = (uintptr_t)sh2_drc_entry;
  if (dispatcher)
    *dispatcher = (uintptr_t)sh2_drc_dispatcher;
  if (exitp)
    *exitp = (uintptr_t)sh2_drc_exit;
}
...
void sh2_drc_mem_setup(SH2_DRC *sh2)
{
...
void sh2_drc_finish(SH2_DRC *sh2)
{
  int i;
