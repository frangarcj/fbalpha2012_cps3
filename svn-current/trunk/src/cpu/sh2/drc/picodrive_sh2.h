...
#define SH2_NO_POLLING	(1 << 10)	// poll detection control
	...
	int	REGPARM(2) (*irq_callback)(struct SH2_DRC_ *sh2, int level);
	int	is_slave;
...
void sh2_reset(SH2_DRC *sh2);
int  sh2_irl_irq(SH2_DRC *sh2, int level, int nested_call);
void sh2_internal_irq(SH2_DRC *sh2, int level, int vector);
void sh2_do_irq(SH2_DRC *sh2, int level, int vector);
void sh2_pack(const SH2_DRC *sh2, unsigned char *buff);
void sh2_unpack(SH2_DRC *sh2, const unsigned char *buff);
...
int  sh2_execute_interpreter(SH2_DRC *sh2c, int cycles);

static __inline void sh2_execute_prepare(SH2_DRC *sh2, int use_drc)
{
#ifdef DRC_SH2
...
static __inline int sh2_execute(SH2_DRC *sh2, int cycles)
{
  int ret;
...
// debug
#ifdef DRC_CMP
void do_sh2_trace(SH2_DRC *current, int cycles);
